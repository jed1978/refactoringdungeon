import type { CombatState, MonsterState, PlayerStats, CombatAction } from "../../utils/types";
...
export function applyPostPlayerAction(
  ...
  if (action.type !== "flee") {
    const primaryIdx = action.type === "attack" || action.type === "skill" ? action.targetIndex : -1;
    if (primaryIdx >= 0) {
      ...
      if (origEnemy && updEnemy && updEnemy.currentHp > 0) {
        ...
        if (origEnemy.def.behavior === "boss_circular_dep") {
          const dmgDealt = events
            .filter((e): e is Extract<typeof e, { kind: "damage_dealt" }> => e.kind === "damage_dealt" && e.targetIndex === primaryIdx)
          ...
          if (dmgDealt > 0) {
            ...
            const rawHp = Math.max(0, (newPlayerStats ?? playerStats).hp - reflectDmg);
            const hp = isDemoMode && rawHp <= 0 ? 1 : rawHp;
            newPlayerStats = { ...(newPlayerStats ?? playerStats), hp };
            if (hp <= 0) events.push({ kind: "combat_lost" });
            log.push(STRINGS.damageReflected.replace("{0}", String(reflectDmg)));
          }
        }
        ...
        if (origEnemy.def.behavior === "boss_god_class") {
          ...
          if (hpPct < 0.75 && bossPhase < 1) {
            bossPhase = 1; events.push({ kind: "boss_phase_shift", newPhase: 1 }); newState = { ...newState, bossPhase };
            log.push(STRINGS.bossPhaseShift.replace("{0}", origEnemy.def.name).replace("{1}", "2"));
          }
          if (hpPct < 0.5 && bossPhase < 2) {
            bossPhase = 2; events.push({ kind: "boss_phase_shift", newPhase: 2 }); newState = { ...newState, bossPhase };
            log.push(STRINGS.bossPhaseShift.replace("{0}", origEnemy.def.name).replace("{1}", "3"));
          }
          if (hpPct < 0.25 && bossPhase < 3) {
            bossPhase = 3; events.push({ kind: "boss_phase_shift", newPhase: 3 }); events.push({ kind: "boss_enrage" }); newState = { ...newState, bossPhase };
    ...
    if (companionCombats > 0) {
      ...
      if (cIdx >= 0) {
        ...
        enemies = applyDamageToEnemy(enemies, cIdx, companionDmg, events, log, origState);
        events.push({ kind: "companion_attack", damage: companionDmg, targetIndex: cIdx });
    ...
    if (aliveEnemies.length === 0) {
      ...
      events.push({ kind: "combat_won", expGained: totalExp, goldGained: totalGold });
      const statsForLevel = newPlayerStats ?? playerStats;
      const withRewards: PlayerStats = { ...statsForLevel, exp: statsForLevel.exp + totalExp, gold: statsForLevel.gold + totalGold };
      const levelResult = checkLevelUp(withRewards);
      newPlayerStats = levelResult.newStats;
      if (levelResult.didLevel) {
        events.push({ kind: "level_up", newLevel: levelResult.newStats.level, unlockedSkills: levelResult.unlockedSkills });
      }
    }
  }
...
export function applyBigBallAbility(
  ...
  if (ability === "self_heal" || ability === "steal_heal") {
    ...
    enemies = enemies.map((e, i) => i === enemyIndex ? { ...e, currentHp: Math.min(e.def.hp, e.currentHp + healAmt) } : e);
    events.push({ kind: "boss_absorb_attack", abilityName: STRINGS.absorbAbilityHeal });
  ...
  if (ability === "multi_hit") {
    events.push({ kind: "boss_absorb_attack", abilityName: STRINGS.absorbAbilityMultiHit });
    log.push(STRINGS.bossAbsorbAttack.replace("{0}", STRINGS.absorbAbilityMultiHit));
  }
  if (ability === "heavy_hit") {
    events.push({ kind: "boss_absorb_attack", abilityName: STRINGS.absorbAbilityHeavyHit });
    log.push(STRINGS.bossAbsorbAttack.replace("{0}", STRINGS.absorbAbilityHeavyHit));
  }
