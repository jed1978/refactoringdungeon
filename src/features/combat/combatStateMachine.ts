import type { CombatState, MonsterState, PlayerStats, Buff } from '../../utils/types';
import { calculateTurnOrder, calculateFleeChance, checkLevelUp } from './combatFormulas';
import { resolvePlayerAttack, resolveSkill, resolveMonsterAttack } from './combatActions';
import { chooseMonsterAction } from './monsterAI';
import { SKILLS_MAP } from '../../data/skills';
import { STRINGS } from '../../data/strings';

export type CombatEvent =
  | { readonly kind: 'damage_dealt'; readonly targetIndex: number; readonly damage: number; readonly isCrit: boolean }
  | { readonly kind: 'damage_received'; readonly damage: number; readonly isCrit: boolean }
  | { readonly kind: 'monster_died'; readonly index: number }
  | { readonly kind: 'monster_split'; readonly sourceIndex: number; readonly clone: MonsterState }
  | { readonly kind: 'skill_used'; readonly skillId: string; readonly mpCost: number }
  | { readonly kind: 'reveal'; readonly index: number }
  | { readonly kind: 'fled' }
  | { readonly kind: 'flee_failed'; readonly damage: number }
  | { readonly kind: 'buff_applied'; readonly buffId: string; readonly turns: number }
  | { readonly kind: 'combat_won'; readonly expGained: number; readonly goldGained: number }
  | { readonly kind: 'combat_lost' }
  | { readonly kind: 'level_up'; readonly newLevel: number; readonly unlockedSkills: readonly string[] };
...
export function processPlayerAction(
  ...
  switch (action.type) {
    case 'attack': {
      ...
      events.push({ kind: 'damage_dealt', targetIndex: targetIdx, damage: result.damage, isCrit: result.isCrit });
      log.push(STRINGS.playerAttacks.replace('{0}', enemies[targetIdx]?.def.name ?? ''));
      ...
      enemies = applyDamageToEnemy(enemies, targetIdx, result.damage, events, log, state);
      break;
    }
    ...
    case 'skill': {
      ...
      events.push({ kind: 'skill_used', skillId: action.skillId, mpCost: skill.mpCost });
      log.push(STRINGS.playerUsesSkill.replace('{0}', skill.name));

      const result = resolveSkill(action.skillId, playerStats, state.selectedTarget, enemies, rng);
      ...
      if (result.kind === 'damage') {
        ...
        if (enemy) {
          log.push(STRINGS.skillRenameVariable
...
export function processEnemyTurn(
  ...
  if (aiResult.shouldSplit) {
    const clone: MonsterState = {
      ...
      buffs: [{ id: 'already_split', name: '已分裂', turnsRemaining: 99, effect: 'split_flag' }],
    };
    const markedMonster: MonsterState = {
      ...monster,
      buffs: [...monster.buffs, { id: 'already_split', name: '已分裂', turnsRemaining: 99, effect: 'split_flag' }],
    };
  ...
  events.push({ kind: 'damage_received', damage: hit.damage, isCrit: hit.isCrit });
...
function applyDamageToEnemy(
  ...
  if (target && target.currentHp <= 0) {
    ...
    log.push(STRINGS.monsterDefeated
