...
function flickerNoise(time: number): number {
  return (
    Math.sin(time * 3.7) * 0.5 +
    Math.sin(time * 5.3) * 0.3 +
    Math.sin(time * 11.1) * 0.2
  );
}

// Reuse a single OffscreenCanvas to avoid allocation every frame
let cachedLightCanvas: OffscreenCanvas | null = null;
let cachedLctx: OffscreenCanvasRenderingContext2D | null = null;
...
function getLightCanvas(
  ...
  if (
    !cachedLightCanvas ||
    cachedLightCanvas.width !== w ||
    cachedLightCanvas.height !== h
  ) {
    cachedLightCanvas = new OffscreenCanvas(w, h);
    cachedLctx = cachedLightCanvas.getContext("2d")!;
  }
  return { canvas: cachedLightCanvas, ctx: cachedLctx! };
}
...
export function renderLighting(
  ...
  const totalRadius =
    (LIGHT_RADIUS_BRIGHT + LIGHT_RADIUS_DIM + flicker) * TILE_SIZE;

  // Reuse cached offscreen canvas
  const { canvas: lightCanvas, ctx: lctx } = getLightCanvas(
    logicalWidth,
    logicalHeight,
  );
