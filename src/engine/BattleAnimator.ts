...
import { createFlash, createShake, updateFlash, updateShake } from './ScreenEffects';
import type { HitSpark } from './BattleRenderer';

export type AnimSequence =
  | { readonly kind: 'idle' }
  | { readonly kind: 'player_attack'; readonly targetIndex: number; elapsed: number }
  | { readonly kind: 'player_skill'; readonly skillId: string; readonly targetIndex: number; elapsed: number }
  | { readonly kind: 'enemy_attack'; readonly enemyIndex: number; elapsed: number }
  | { readonly kind: 'hit_reaction_enemy'; readonly targetIndex: number; elapsed: number }
  | { readonly kind: 'hit_reaction_player'; elapsed: number }
  | { readonly kind: 'death_dissolve'; readonly targetIndex: number; elapsed: number }
...
export function queueAnimation(state: BattleAnimState, seq: AnimSequence): BattleAnimState {
...
export function updateAnimation(
  ...
  let screenFlash = state.screenFlash ? updateFlash(state.screenFlash, dt) : null;
  const screenShake = state.screenShake ? updateShake(state.screenShake, dt) : null;
  ...
  const defaultOffsets: SpriteOffset[] = Array.from({ length: enemyCount }, () => ({ x: 0, y: 0 }));
  const defaultFlash: boolean[] = Array.from({ length: enemyCount }, () => false);
  ...
  switch (current.kind) {
    ...
    case 'player_attack': {
      ...
      if (t >= 0.45 && t < 0.55 && hitSparks.length === 0) {
        hitSparks = [...hitSparks, createHitSparkAt(current.targetIndex, enemyCount)];
      }
      if (t >= 0.5 && t < 0.6) {
    ...
    case 'hit_reaction_enemy': {
      ...
      if (idx < enemyCount) {
        ...
        enemyOffsets[idx] = { x: Math.round(Math.sin(t * Math.PI * 4) * 2), y: 0 };
      }
      finished = current.elapsed >= 300;
      break;
    }
